...
module "create_info_stream" {
  ...
  account_id  = data.aws_caller_identity.aggregate.account_id
  stream_name = "${terraform.env}-${var.es_index}-info"
  s3_bucket   = var.s3_bucket
  s3_prefix   = "AWSLogs/${data.aws_caller_identity.aggregate.account_id}/${var.es_index}/info/${terraform.env}/"
  es_region   = var.aggregate_region
...
module "create_error_stream" {
  ...
  s3_bucket   = var.s3_bucket
  s3_prefix   = "AWSLogs/${data.aws_caller_identity.aggregate.account_id}/${var.es_index}/error/${terraform.env}/"
  es_region   = var.aggregate_region
  es_domain   = var.es_domain
  es_index    = var.es_index
  es_type     = "error"
...
module "create_delivery_func" {
  ...
  providers = {
    aws.src = aws.sender
    aws.dst = aws.aggregate
  }

  src_region                      = var.sender_region
  dst_region                      = var.aggregate_region
